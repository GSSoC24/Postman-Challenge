...
function App() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('https://raw.githubusercontent.com/GSSoC24/Postman-Challenge/main/add-your-certificate.md');
                const parsedData = parseMarkdownTable(response.data);
                const referralData = await fetchReferralData();
                const updatedLeaderboard = calculateLeaderboard(parsedData, referralData);
    ...
    const calculateLeaderboard = (parsedData, referralData) => {
        ...
        return Object.values(referralCount).sort((a, b) => b['Number of Certifications'] - a['Number of Certifications']);
    };
